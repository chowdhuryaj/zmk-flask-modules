...
#define FLASK_COMBOS_SLOTS 32
#define FLASK_COMBOS_KEYS 4
#define FLASK_COMBOS_POS_NONE 0xFF

/* One runtime combo: up to 4 key positions (0xFF = unused) and the encoded
 * HID usage it emits (ZMK keymap encoding: usage id bits 0-15, usage page
 * bits 16-23, modifiers bits 24-31). A slot is live when usage != 0 and at
 * least two positions are set. */
...
/* Persist the live table (CMD_SAVE path; settings key "flask/combos"). */
int flask_combos_save(void);

/* Restore hook — called from flask_proto.c's "flask" settings handler for
 * the "combos" leaf. Size/version mismatches are ignored (table stays
 * empty; nothing to reseed — defaults ARE empty). */
